...
_plot_colors = ('r', 'g', 'b', 'c', 'darkorange', 'black', 'purple', 'y')

from helpers.clustering import r_clusters, c_clusters

def tmp():
    # SSE
    x, c_y, r_y = read_csv('./OUTPUT/BASE/SSE.csv'.format(OUT), 3)
    f = Figures("Sum of Squared Error - Cancer", "K Clusters", "Squared Error")
    f.start()
    f.plot_curve("cancer", x, c_y)
    f.finish()
    f = Figures("Sum of Squared Error - Reviews", "K Clusters", "Squared Error")
    f.start()
    f.plot_curve("reviews", x, r_y)
    f.finish()

def compare_between_params(_file, title, y_axis, x_axis, it, OUT=OUT):
    def plot(d):
        _f = Figures("%s - %s" % (title, d), y_axis, x_axis)
        _f.start()
        _x = c_clusters
        for i in it:
            _, c, r = read_csv('./OUTPUT/{}/{}/{}'.format(OUT, i, _file), 3)
            _f.plot_curve(str(i), _x, interpolate_gaps(c if d == 'Cancer' else r), plot_colors=_plot_colors)
        _f.finish()
    plot('Cancer')
    plot('Reviews')

def acc_between_params(_file, title, y_axis, x_axis, it, OUT=OUT):
    def plot(alg):
        _f = Figures("%s %s" % (alg, title), y_axis, x_axis)
        _f.start()
        _x = c_clusters
        for i in it:
            _, g, k = read_csv_sideways('./OUTPUT/{}/{}/{}'.format(OUT, i, _file))
            _f.plot_curve(str(i), _x, interpolate_gaps(g[1] if alg == 'EM' else k[1]), plot_colors=_plot_colors)
        _f.finish()
    plot('EM')
    plot('KMeans')

def acc_between_params_same(_file, title, y_axis, x_axis, it, OUT=OUT):
...
if __name__ == '__main__':
    ...
    acc_between_params_same('reviews_adjMI.csv', "Mutual Information - ICA Reduced - Reviews", "K Clusters/Components", "Adjusted Mutual Information", [13, 21], OUT="ICA")
    acc_between_params_same('cancer_adjMI.csv', "Mutual Information - ICA Reduced - Cancer", "K Clusters/Components", "Adjusted Mutual Information", [13, 61], OUT="ICA")

    plt.show()
    sys.exit(0)

    # ICA Mutual Information
    acc_between_params('reviews_adjMI.csv', "Mutual Information - ICA Reduced - Reviews", "N Components", "Adjusted Mutual Information", [5, 13, 21, 29, 37, 45, 53, 61], OUT="ICA")
    acc_between_params('cancer_adjMI.csv', "Mutual Information - ICA Reduced - Cancer", "N Components", "Adjusted Mutual Information", [5, 13, 21, 29, 37, 45, 53, 61], OUT="ICA")
    ...
    # ICA Accuracy
    acc_between_params('reviews_acc.csv', "Accuracy - ICA Reduced - Reviews", "K Clusters", "Accuracy", [5, 13, 21, 29, 37, 45, 53, 61], OUT="ICA")
    acc_between_params('cancer_acc.csv', "Accuracy - ICA Reduced - Cancer", "K Clusters", "Accuracy", [5, 13, 21, 29, 37, 45, 53, 61], OUT="ICA")

    # PCA Mutual Information
    acc_between_params('reviews_adjMI.csv', "Mutual Information - PCA Reduced - Reviews", "K Clusters", "Accuracy", [0.6, 0.7, 0.8, 0.9], OUT="PCA")
    acc_between_params('cancer_adjMI.csv', "Mutual Information - PCA Reduced - Cancer", "K Clusters", "Accuracy", [0.6, 0.7, 0.8, 0.9], OUT="PCA")

    acc_between_params('reviews_acc.csv', "PCA Reduced - KMeans Accuracy - Reviews", "K Clusters", "Accuracy", [0.6, 0.7, 0.8, 0.9], OUT="PCA")
    acc_between_params('cancer_acc.csv', "PCA Reduced - KMeans Accuracy - Reviews", "K Clusters", "Accuracy", [0.6, 0.7, 0.8, 0.9], OUT="PCA")

    # SSE
    compare_between_params('SSE.csv', "PCA - Sum of Squared Error", "K Clusters", "Squared Error", [0.6, 0.7, 0.8, 0.9], OUT="PCA")
    compare_between_params('logliklihood.csv', "PCA - Log Likelihood", "K Clusters", "Log Likelihood", [0.6, 0.7, 0.8, 0.9], OUT="PCA")

    compare_between_params('SSE.csv', "ICA - Sum of Squared Error", "K Clusters", "Squared Error", [5, 13, 21, 29, 37, 45, 53, 61], OUT='ICA')
    compare_between_params('logliklihood.csv', "ICA - Log Likelihood", "K Clusters", "Log Likelihood", [5, 13, 21, 29, 37, 45, 53, 61], OUT='ICA')
